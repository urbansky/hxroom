...
**In der Oberfläche** liegt der Zustand je App in `useCallChat()` neben `useCallState()` – dieselbe Teilung wie bei den Notizen, denn der Tab-Wechsel baut das Panel ab. Eine eigene Nachricht steht sofort da und trägt „sendet …", bis die gespeicherte Fassung sie ersetzt; scheitert das Senden, steht „Nicht gesendet" mit „Erneut senden". Eine Nachricht, von der man glaubt, sie sei angekommen, wäre im Tonausfall das Schlimmste. Kommt etwas an, während der Chat zu ist, erscheint ein Punkt am Reiter **und** ein kurzer Hinweis über der Bühne mit „Chat öffnen" – wer den Ton verloren hat, schaut auf das Bild und nicht in die Seitenleiste. Das Mail-Symbol an den Nachrichten ist weg, und der erklärende Hinweis über dem Verlauf ebenso: Die Zusammenfassungsmail, die er versprach, gibt es nicht, und den Zweck des Chats erklärt die Seitenleiste von selbst.

**Links sind anklickbar** – einer der beiden Zwecke des Chats. Erkannt werden `http(s)://…` und `www.…`; ein nacktes „hxroom.de" bleibt Text, weil sich eine Domain nicht zuverlässig von „z.B." oder „Mo.-Fr." unterscheiden lässt. Satzzeichen und eine schließende Klammer am Ende gehören zum Satz, nicht zur Adresse. Gebaut ohne `v-html`: Die Teile stehen als Text im Baum, eine Nachricht kann also kein Markup einschleusen, und `javascript:` erfasst das Muster gar nicht erst. Die Links öffnen in einem neuen Tab, mit `rel="noopener noreferrer"`.
...
#### Nachtrag: Teil 2 – Dateien *(umgesetzt 2026-09-24)*

Coach und Klient können eine Datei teilen. Sie geht über die API, liegt im Objektspeicher und kommt über einen kurzlebigen, signierten Link wieder heraus.

**Tabelle `session_chat_files`**, eine Datei je Nachricht, ein Begleittext ist optional. Das Objekt liegt unter `{organizationId}/sessions/{bookingId}/attachments/{fileId}.{ext}` – ohne echten Dateinamen, der steht in der Datenbank. Oberstes Segment ist die Organisation, damit die Kontolöschung die Anhänge mit einem Prefix-Delete erfasst.

**Geprüft wird, was wirklich ankommt** (`call-file-type.ts`): Endung **und** Dateisignatur müssen zusammenpassen, und gespeichert wird der so ermittelte Typ, nicht der vom Browser gemeldete. Eine als PDF benannte HTML-Datei kommt damit nicht in den Speicher. docx, xlsx und pptx sind ZIP-Archive und an der Signatur nicht zu unterscheiden – die Endung entscheidet, welches der drei es ist; Makro-Formate stehen nicht auf der Liste. **Bilder werden über `sharp` neu kodiert und verlieren dabei ihre Metadaten:** Ein Handyfoto trägt sonst den Aufnahmeort des Klienten. Verkleinert wird nicht, weil ein abfotografiertes Dokument lesbar bleiben soll.

**Hochgeladen wird über die API**, nicht direkt in den Speicher: Sonst läge die Datei schon im Bucket, bevor irgendetwas geprüft ist. **Heruntergeladen** wird über eine Weiterleitung – der Verlauf trägt eine dauerhafte Adresse auf die API, sie prüft beim Klick und antwortet mit `302` auf einen für 15 Minuten signierten Link. Was der Browser damit tut, steht in der Signatur: `attachment` mit dem ursprünglichen Namen (zusätzlich als `filename*`, sonst verliert ein Name mit Umlauten seine Zeichen), der geprüfte Typ und `no-store`.

**Gefunden dabei, gemeldet aus dem Gebrauch:** Das Herunterladen beendete das Gespräch. Der Link lag im selben Tab, und Chrome feuert beim Navigationsversuch `beforeunload` und `pagehide`, noch bevor es die Antwort als Download erkennt und die Navigation wieder abbricht. `livekit-client` hängt genau daran (`disconnectOnPageLeave`) und trennt den Raum – die Seite blieb stehen, die Spuren waren `ended`. Gemessen: Vor dem Klick vier laufende Spuren, danach zwei beendete und die Wiedergabezeit auf 0. Der Link trägt deshalb `target="_blank"`; damit wird das Dokument nie verlassen. Das `download`-Attribut ist entfallen, es wirkt bei fremder Herkunft ohnehin nicht – den Namen trägt der Content-Disposition-Header.

**Gefunden dabei:** multer liefert den Dateinamen als Latin-1 – „Fragebogen für Markus.pdf" landete als „Fragebogen fÃ¼r Markus.pdf" in der Datenbank. Das Multipart-Format sieht keine Angabe zur Kodierung vor, busboy entscheidet sich deshalb für Latin-1. `safeFileName()` liest die Bytes zurück als UTF-8, außer es entstehen dabei Ersatzzeichen.

Grenzen: 25 MB je Datei, 20 Dateien je Sitzung. Die Größe fängt schon multer ab – die Antwort ist dann `413`, nicht `400` wie sonst bei einer abgelehnten Datei. Die Oberfläche prüft Größe und Endung deshalb vorab, damit niemand 25 MB hochlädt, um danach ein „geht nicht" zu lesen.

**In der Oberfläche** hängt eine Büroklammer neben dem Eingabefeld; die gewählte Datei geht sofort hinaus, ein bereits getippter Text wird ihr Begleittext. **Dateien lassen sich auch auf das Chat-Panel ziehen** – eine gestrichelte Fläche zeigt, wo sie landen; mehrere auf einmal werden zu mehreren Nachrichten, weil der Server eine Datei je Nachricht annimmt. Reagiert wird nur auf Dateien, ein hineingezogener Text oder Link bleibt Sache des Eingabefelds. Die ganze Call-Seite fängt außerdem Dateien ab, die *neben* dem Panel losgelassen werden: Der Browser öffnete sie sonst im selben Tab, und das wäre derselbe Abbruch wie beim Herunterladen (siehe unten). Geprüft mit im Browser erzeugten Drag-Ereignissen; ob Chrome bei einem echten Fehlwurf aus dem Finder tatsächlich stehen bleibt, lässt sich so nicht automatisieren – die Prüfung belegt nur, dass die Seite das Ereignis abfängt. Die Datei steht als eigene Zeile unter der Nachricht, mit Name und Größe, und ist erst anklickbar, wenn sie liegt. Was nicht durchkommt, meldet beim Coach ein Toast und beim Klienten – der hat keine Toasts – ein Hinweis über der Bühne. Ein zweiter Versuch ist möglich, solange die Datei noch im Speicher des Browsers liegt; nach einem Reload ist sie weg, und die Nachricht mit ihr.

Abnahme auf API-Ebene: PDF mit Begleittext geteilt, derselbe `clientMessageId` zweimal ergibt eine Nachricht, Weiterleitung mit `302` und `no-store`, die Datei kommt mit `attachment`, richtigem Namen und geprüftem Typ an; ein Foto mit EXIF liegt ohne Metadaten im Speicher; umbenannte HTML-Datei, `.docm` und 26 MB werden abgewiesen; unbekannte `fileId` 404, falscher Token 401; nach dem Ende lädt der Coach weiter, der Klient nicht mehr; die 21. Datei wird abgelehnt. Im Browser mit beiden Seiten: Coach hängt ein PDF an, der Klient sieht es, lädt es herunter – mit Umlaut im Dateinamen – und kann es nach einem Reload erneut laden; eine `.txt` meldet sich sofort, ohne Anfrage an den Server.

**Bilder und PDFs werden angezeigt statt heruntergeladen** *(Nachtrag 2026-09-25)*. Ein Foto, das im Gespräch geteilt wird, will man jetzt sehen und nicht im Download-Ordner suchen.

- **Im Chat steht ein Vorschaubild**, das beim Hochladen entsteht: WebP, längste Kante 480 px, aus dem bereits bereinigten Bild, also ebenfalls ohne Metadaten. Gemessen: 12 KB statt 4,8 MB beim Original. Das Original lädt erst in der Großansicht – sonst lüde ein Handyfoto mitten im Videocall über dieselbe Leitung wie das Video. Die Maße gehen mit in die Antwort (`preview_width`/`preview_height`), damit der Chat den Platz reserviert; ein nachladendes Bild verschöbe sonst den Verlauf, und das Mitscrollen ans Ende liefe ins Leere.
- **Ein Klick öffnet die Großansicht** (`CallImageViewer`) mit allen Bildern der Sitzung: blättern mit Pfeilen, Pfeiltasten oder Wischen, „2 von 3", und ein Knopf, der das Original in einem neuen Tab öffnet. Keine eigene Slideshow – wer Bilder vorführen will, hat die Bildschirmfreigabe, und der Chat bleibt die Rückfallebene.
- **PDFs öffnet der Viewer des Browsers in einem neuen Tab**, Office-Dateien werden weiter heruntergeladen. Der Dienst entscheidet das nach dem geprüften Typ (`inline` bzw. `attachment`). `inline` ist hier vertretbar, weil im Speicher nur geprüfte Typen liegen und Bilder zusätzlich neu kodiert sind; SVG ist nicht erlaubt.
- **Bilder dürfen im Browser liegen bleiben.** Der Tab-Wechsel baut den Chat jedes Mal neu auf, ohne Cache lüden alle Vorschaubilder jedes Mal erneut. Eine Signatur hängt aber am Zeitpunkt, jeder Abruf ergäbe eine neue URL und damit keinen Treffer im Cache. Bei Bildern wird deshalb auf den Beginn eines Zehn-Minuten-Fensters signiert: Innerhalb des Fensters entsteht dieselbe URL, die Weiterleitung darf bis zum Fensterende gemerkt werden, das Objekt zehn Minuten. Gültig bleibt der Link 15 Minuten ab Fensterbeginn. RustFS nimmt die rückdatierte Signatur an; das ist gegen Hetzner beim Umstieg noch zu prüfen. Dokumente bleiben bei `no-store` und frischem Link.

**Gefunden dabei:** Die Großansicht lag *unter* der Seitenleiste und der Steuerleiste des Calls – das Bild halb verdeckt, der Schließen-Knopf unsichtbar. Nuxt UI gibt dem Modal keine eigene Ebene, sondern verlässt sich auf die Reihenfolge im DOM; `CallImageViewer` setzt deshalb ausdrücklich `z-[100]`. Außerdem legt `UCarousel` Pfeile und Punkte außerhalb seiner Fläche ab (`sm:-start-12`, `-bottom-7`); bei voller Breite waren sie abgeschnitten und sind jetzt hereingeholt.

Abnahme auf API-Ebene: Foto 3000×2000 ergibt eine Vorschau 480×320, WebP, 12 KB, ohne EXIF, `inline`, `private, max-age=600`; zwei Abrufe hintereinander liefern dieselbe signierte URL; das Original kommt `inline` in voller Größe; PDF `inline` mit `no-store` und ohne Vorschau-Adresse (404); Office weiter `attachment`; nach dem Ende bekommt der Klient auch die Vorschau nicht mehr. Im Browser beim Klienten: drei Vorschaubilder geladen, jeweils höchstens 480 px, Hochformat bleibt hochkant; Klick auf das zweite öffnet „2 von 3" mit dem Original; Pfeiltasten, Pfeilknöpfe und Punkte funktionieren; Escape schließt; das PDF öffnet einen neuen Tab mit `inline`; nach Großansicht und PDF laufen alle Videospuren weiter.

**Übersicht der geteilten Dateien** *(Nachtrag 2026-09-25)*. In einem längeren Verlauf sind Dateien schnell weggescrollt – dabei sind sie das, was vom Chat bleibt. Sobald die erste geteilt ist, steht oben im Chat-Panel eine Zeile „3 Dateien geteilt"; sie führt zu einer Übersicht mit den Bildern als Raster (ein Klick öffnet die Großansicht) und den Dokumenten als Liste mit Größe, Absender und Zeit. „Im Verlauf zeigen" springt von einem Dokument zu seiner Nachricht und hebt sie kurz hervor; „Zurück zum Verlauf" führt dorthin, wo man war, und wer vorher unten stand, landet wieder unten. Wer in der Übersicht schreibt oder eine Datei hineinzieht, kommt zurück in den Verlauf, ans Ende.

- Im Panel und nicht in der Titelzeile: Die gehört dem rollenfreien Gerüst des Calls und trägt ebenso „Notizen" und „Klient".
- Kein Karussell für alles: PDF und Office lassen sich darin nicht zeigen, dort stünde nur ein Symbol. Das Karussell ist die Großansicht der Bilder, die es schon gibt.
- Kein eigener Reiter „Dateien": Beim Klienten gibt es nur den Chat, und ein zweiter Knopf in der Steuerleiste kostet auf dem Handy Platz.
- Der Verlauf bleibt aufgebaut (`v-show`), solange die Übersicht offen ist – sonst begänne er beim Zurückkehren oben.

Abnahme im Browser mit vier früh geteilten Dateien und 26 späteren Nachrichten: Die Zeile nennt „4 Dateien", die Übersicht „Bilder · 2" und „Dokumente · 2"; eine Kachel öffnet die Großansicht beim richtigen Bild, danach steht die Übersicht wieder da; „Im Verlauf zeigen" bringt das PDF in den sichtbaren Bereich und hebt es hervor; „Zurück zum Verlauf" landet wieder unten; Senden aus der Übersicht führt ans Ende zur eigenen Nachricht. Beim Klienten dieselbe Übersicht, die Vorschaubilder geladen, seine Links mit Token.

Bewusst nicht dabei: Löschen einer geteilten Datei, ein Virenscan und der Verlauf im Termin-Slideover – die Übersicht ist so gebaut, dass sie dort später wiederverwendet werden kann.

**Gefunden beim Testen mit vielen Dokumenten** *(2026-09-25)*: Dem Klienten fehlte eine Datei, bis er neu lud. Das Nachholen beruht auf einem Zeiger – „gib mir alles nach Nummer N" –, und an drei Stellen konnte er über etwas hinwegspringen, das noch nicht abgeholt war:

1. **Nachricht und Datei wurden in zwei Befehlen geschrieben.** Wer genau dazwischen nachholte, bekam die Nachricht ohne Datei und rückte den Zeiger darüber hinweg; die Datei kam nie nach. Das war der gemeldete Fall. Jetzt entstehen beide in einer Transaktion.
2. **Nummern werden beim Einfügen vergeben, sichtbar wird eine Zeile aber erst beim Abschluss.** Zwei gleichzeitige Nachrichten konnten als 11 vor 10 erscheinen, und 10 wäre übersprungen worden. Jetzt sperrt jede Chat-Transaktion die Buchungszeile (`FOR UPDATE`): Je Sitzung wird eine Nachricht nach der anderen vergeben und abgeschlossen, Sichtbarkeit und Nummernfolge stimmen überein. Andere Sitzungen berührt das nicht; das Hochladen in den Speicher liegt vor der Sperre. Nebenbei hält damit auch die Grenze von 20 Dateien bei gleichzeitigen Uploads.
