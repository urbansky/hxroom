...
**Herkunft der Schicht:** `packages/livekit` wird nicht neu geschrieben, sondern aus **HxMeet** (`hxmeet-core-component`, MIT-Lizenz, eigene Vorarbeit) übernommen und auf den HxRoom-Scope entkernt – Chat, Reactions, Teilnehmerliste und die Multi-Party-Layouts entfallen, siehe `project.md` §5a. Mitgenommen wird vor allem die erprobte LiveKit-Mechanik: `prepareConnection()` als Warmlauf schon im Warteraum, browserspezifische Behandlung verweigerter Kamera-/Mikrofonfreigaben (Chrome, Safari und fehlende Hardware melden sich unterschiedlich), Connect-Retry und die Auswertung der `DisconnectReason`. Die Oberfläche von HxMeet ist mit B3 vollständig entfallen, weil HxRoom inzwischen eine eigene hat; die Frage der Nuxt-UI-Version hat sich damit erledigt, das Paket enthält nur noch TypeScript und hat außer `vue` keine Peer-Abhängigkeit mehr.

Die Teilnehmer bleiben dabei als **Liste** modelliert, obwohl HxRoom 1:1 führt: Wer der lokale ist, sagt `localIdentity`. Ein auf zwei Personen verdrahtetes Modell müsste für jeden weiteren Teilnehmer wieder aufgemacht werden.
...
| 04 | **Geteilte Dateien: Löschen und Virenscan** | Eine im Call geteilte Datei (B7) lässt sich nicht einzeln entfernen – ein versehentlich geteiltes Dokument bleibt bis zur Kontolöschung liegen. Geprüft werden Endung, Signatur und Größe, ein Virenscan (etwa ClamAV) fehlt; Coachs öffnen Dateien, die Klienten hochgeladen haben. | Vor dem Start mit echten Coaches |
