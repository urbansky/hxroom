...
Die Call-Oberfläche liegt rollenfrei in `packages/ui/components/call/` und trägt beide Seiten: `CallScreen` als Gerüst (Bühne, Steuerleiste, Slideover), dazu `CallVideoArea`, `CallControls`, `CallChatPanel`, `CallCameraView` und `CallAudioOutput`. Was ein Browser nicht kann oder was noch nicht gebaut ist, bleibt hinter Schaltern ausgeblendet (`can-share` je nach `screenShareSupported()`, `can-blur` bis zum Weichzeichnen) – im echten Gespräch soll nichts eine Wirkung vortäuschen. Rollenspezifisches – Notizen, Klientenakte, Stummschalten, Timer, das Beenden-Modal – bleibt in der jeweiligen App und wird als Props und Slots hineingereicht (`#sidebar`, `#stage-overlay`). Einen Extension-Seam oder Event-Bus gibt es dafür bewusst nicht.
